...
Projekt systemu składał się z dwóch równie istotnych części : schemat reprezentacji użytkowników oraz schemat przebiegu realizacji projektu.\\
...
Uważaliśmy, iż zwykli pracownicy nie powinni znać klienta i całego projektu (za wyjątkiem kierowników i menadżerów),a tylko powinni mieć dostęp do informacji zawartych w Zadaniu.
 Z drugiej strony klient nie powinien znać wszystkich szczegółów realizacji projektów, a jedynie efekty poszczególnych etapów (Prac).
...
Postanowiliśmy zatem, aby potrakotwać zarówno Ofertę, jak Projekt jako pudełka, które służą do zarządzania zleceniem, którego poszczególne etapy (części) znajdują się w liście Prac.
To w Pracy miały zawierać się wszystkie techniczne informacje dotyczące tłumaczenia (pliki, deadline, rodzaj). I tak np. na poziomie Oferty  znajdują się dane o kliencie,
...
Nie wiedzieliśmy natomiast ilu takich Menadżerów przypisać oraz czy powinni oni być powiązania stricte z Pracą , czy wyżej z Projektem. 
Po długich dyskusjach i licznych pomysłach zdecydowaliśmy się na najbardziej elastyczny model. Ilość menadżerów miała zależeć od Kierownika zlecenia, który przygotował Ofertę. 
...
\end{itemize}
Przedstawione wyżej różne problemy, jakie natrafiliśmy na etapie projektowania są tylko częścią tego z czym musieliśmy się zmierzyć. 
Uważamy, że zrealizowany przez nas schemat spełnia wszystkie wymagane funkcjonalności oraz pozwala na wystarczającą zdolność przystosowania się.
