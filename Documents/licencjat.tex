...
Następnie kierownik oferty jest zobowiązany do podzielenia pracy na poszczególne etapy. W tym celu tworzy Prace (ang. Jobs), które zawierają:
\begin{enumerate}
	\item	Przydzielone pliki
	\item 	Opis
	\item	Termin zakończenia pracy
	\item 	Cenę
\end{enumerate}
\begin{figure}[h!]
\centering
...
Menadżerowie projektów zgodnie z ustalonym podziałem pracy są zobowiązani do podzielenia kolejnych etapów na zadania, które zlecają tłumaczom i weryfikatorom. Są oni odpowiedzialni za ustalenie terminu przyjęcia rozwiązania, przydzielenie odpowiedniego pliku do tłumaczenia oraz ostatecznej oceny pracy zarówno tłumacza, jak i weryfikatora.Szczegółowy opis etapu tłumaczenia zostanie opisany w dalszej części.\\
Po zakończeniu etapu, tzn. przetłumaczeniu całego tekstu załączonego do Pracy Menadżerowie Projektu są zobowiązani do przygotowania ostateczenej wersji pliku i udostępnienie jej Klientowi. Klient może za pomocą pasku stanu zaakceptować tak wykonane zadanie, bądź może poprosić o dokonanie odpowiednich poprawek.

\item Zakończenie  zlecenia - Klient akceptując wykonanie etapu zlecenia (Pracy) jest zobowiązany do rozliczenia jej. Gdy takowe nastąpi księgowy zaznacza ten fakt i możliwe jest zakończenie przez Kierownika przebiegu Pracy. Gdy zostaną zakończone wszystkie Prace następuje rozliczenie i ostateczne zamknięcie całej Oferty. Zamknięta Oferta wraz ze wszystkimi dokumentami trafia następnie do archiwum. 
\end{itemize}  

\section{Podstawowy przypadek użycia}
Uznaliśmy, że najważniejszym przypadkiem użycia wymagającym głębszego wyjaśnienia jest etap tłumaczenia. Przedstaiwony zostanie on bardziej szczegółowo, aby wyjaśnić wszelkie wątpliwości i ukazać możliwości naszej aplikacji.

\begin{enumerate}
\item	Tworzenie Zadania - Menadżer Projektu odpowiedzialny za daną Pracę tworzy nowe Zadanie wybierając termin, jego realizacji, cenę i walutę w jakiej zostanie rozliczone zadanie oraz opis wyjaśniający problem . Ponadto Menadżer wybiera Tłumacza i Weryfikatora z listy pracowników przydzielonych do Projektu, którzy odpowiadają profilowi zadania. \\
\begin{figure}[h!]
\centering
...
\item Zakończenie - Po rozliczeniu Zadania Menadżer Projektu może go zakończyć a wszystkie pliki i informacje trafiają do archiwum. 
\end{enumerate}
...
przeprowadzili wspólnie Karol Nienałtowski oraz Jakub Oćwieja, natomiast przedstawienie technologii przy użyciu których będziemy realizować projekt, przypadło prze Michał Sabaciński i Krzysztof Gogolewski.
...
\subsection{funkcjonalności dot. procesu tłumaczenia}
...
\subsection{funkcjonalności dot. administracji systemu}
