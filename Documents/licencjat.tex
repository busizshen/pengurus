...
\kierownictwo{dra Janusza Jabłonowskiego\\Instytut Informatyki}
...
Są one jednak ograniczone jedynie do pracowników biura i nie jest możliwa praca mobilna (\textit{quahill}),
bądź w przypadku aplikacji webowych zostały stworzone przy użyciu starych technologii, takich jak PHP (\textit{]project-open[}). 
Ponadto produkty te często zapewniają niski poziom bezpieczeństwa, czy też nie są przeznaczone do kontaktów z klientami (\textit{Projetex}).
Pengurus CRM ma być aplikacją, która wychodzi naprzeciw tym problemom. 
Będzie to pierwsza na rynku aplikacja WWW stworzona przy użyciu gamy nowych technologii - ExtGWT, Hibernate, Spring.
...
\section{Użytkownicy}
\begin{itemize}
\item Klient - podmiot zlecający tłumaczenie firmie. Rozróżniamy dwa rodzaje klientów
\begin{itemize}
\item Klient Indywidualny (ang. Individual Client).
\item Klient Biznesowy (ang. Bussines Client) - w szczególności klient biznesowy może posiadać pełnomocników zajmujących się zleceniem.
\end{itemize}
\item Tłumacz (ang. Translator) - podmiot wykonujący tłumaczenia bądź weryfikacje. W szczególności może pełnić role Pracownika. Wyróżniamy różne typy Tłumaczy:
\begin{itemize}
\item Ekspert (ang. Expert) – tłumacz zatrudniony w firmie bądź wykonujący zlecenie, odpowiedzialny za tłumaczenie dokumentów wyznaczanych przez Menadżera Projektu.
\item Pracownik nieetatowy (ang. Freelancer) – osoba nie będąca na stałe zatrudniona w firmie, która na potrzeby chwili wykonuje odpłatnie zlecone tłumaczenia.
\end{itemize}  
\item Pracownik (ang. Worker) - pracownik firmy, który nie jest Tłumaczem.  
\begin{itemize}
\item Kierownik (ang. Executive) – Menadżer ds. produkcji/Stanowisko ds. relacji z klientami i produkcji. Kierownik zlecenia. 
Podmiot posiadający najwięcej praw w systemie Pengurus, fizycznie rola przewidziana dla osób postawionych najwyżej w hierarchii firmy.
Jako jedyny posiada dostęp do panelu administratora.
\item Menadżer Projektu (ang. Project Manager)– Lider dla tłumaczenia. Osoba odpowiadająca za nadzorowanie projektów.
\item Księgowy (ang. Accountant) – osoba odpowiedzialna za księgowość firmy.
\end{itemize}
\end{itemize}

\section{Przypadki użycia}
\subsection{Przyjęcie zlecenia}

Kierownik jest odpowiedzialny za przyjęcie zlecenia od klienta oraz przygotowanie odpowiedniej oferty, 
która następnie zostaje poddana ocenie klienta.

Kierownik odbiera dokumenty od klienta wraz z tekstem oryginału, zapoznaje się on z przesłanym/odebranym materiałem,
...
Ponadto następuje przygotowanie dziennik zlecenia. W dzienniku zapisywana jest historia pracy z dokumentem i 
...
\subsection{Rejestracja zlecenia}
Kierownik zlecenia wyznacza Menadżera Projektu. W uzasadnionych przypadkach działania Menadżera Projektu wykonuje Kierownik zlecenia.
Ponadto jest on odpowiedzialny za przypisanie odpowiednich tłumaczy/weryfikatorów do projektu oraz udostępnia menadżerom przesłane przez klienta zasoby (pliki) i wyznacza im termin realizacji poszczególnych etapów projektu.

\subsection{Tłumaczenie i weryfikacja}
...
Ponadto menadżer wyznacza odpowiedniego Weryfikatora.
Tłumacz przesyła pliki ze zrealizowanymi etapami pracy. Każdy z tych plików może być edytowany przez autora, a dostęp do nich posiadają także menadżerowie oraz weryfikatorzy.
...
\subsection{Przekazanie gotowego tłumaczenia do zleceniodawcy}
Menadżer Projektu odbiera od Tłumaczy pliki oraz przygotowuje ostateczny dokument, który następnie zostaje podany ocenie Kierownikowi. 
Po zaakceptowaniu projekt trafia do Klienta, który poddaje go swojej ocenie, a w razie reklamacji dokument wraz z uwagami zostaje przesłany 
Menadżerowi Projektu do ponownego rozpatrzenia.

\subsection{Archiwizacja zleceń}
...
\subsection{Tworzenie i edycja użytkowników systemu}
Kierownik tworzy użytkownika oraz nadaje mu odpowiednie role.
Pracownik/Tłumacz posiada: login, dane osobowe i kontaktowe. 
Tłumacz ma również przypisane odpowiednie umiejętności tłumaczenia (wraz z prawami do ich tworzenia - tłumaczenia sądowe, techniczne).
Klient posiada login, dane kontaktowe oraz dane służące do przygotowania umowy i faktur. W przypadku Klienta biznesowego każdy, z jego pełnomocników posiada dane osobowe i kontaktowe. 


\chapter{Aplikacja}\label{r:aplikacja}
...
System Zarządzania Biurem Tłumaczeń Pengurus jest aplikacją przeznaczoną dla użytkowników pełniących różne role w procesie tłumaczenia.
W zależności od pełnionej roli użytkownik aplikacji posiada specyficzną bazę uprawnień do zarządzania danymi.
 W szczególności niektórzy użytkownicy mają zablokowaną możliwość podglądu, bądź edycji niektórych danych.
...
Wszystkie dozwolone operacje - dane do których istnieje dostęp mają odnośnik.
\item  Panel główny, w którym wyświetlane są akcje prowadzone przez użytkownika.  
\begin{figure}[ht!]
\centering
...
Ponadto każdy Kierownik ma prawo do wglądu i edycji danych osobowych każdego z użytkowników. 
Kierownicy będący jednocześnie administratorami systemu zarządzają aspektami technicznymi.\\
...
\item  Administrowanie językami - edytowanie oferty dostępnej przez biuro tłumaczeń, dodawanie i edycja języków oraz dostępnych typów tłumaczeń (np. tłumaczenia sądowe, naukowe - informatyczne), przypisywanie stałych cen za jednostkę pracy,
...
\end{figure}
\end{itemize}
\section{Realizacja przebiegu zlecenia w aplikacji}
...
Po zakończeniu pojedynczej pracy, tzn. przetłumaczeniu całego tekstu załączonego do pracy, menadżerowie projektu są zobowiązani do przygotowania ostatecznej wersji pliku i udostępnienie jej klientowi. Klient może za pomocą pasku stanu zaakceptować tak wykonaną pracę, bądź może poprosić o dokonanie odpowiednich poprawek.
...
\section{Podstawowy przypadek użycia}\label{r:tlumaczenie}
Podstawowym przypadkiem użycia jest etap tłumaczenia - Zadanie. Przedstawiony zostanie on bardziej szczegółowo, aby wyjaśnić wszelkie wątpliwości i ukazać możliwości naszej aplikacji.
\begin{figure}[ht!]
\centering
\includegraphics[width=0.7\textwidth]{resources/tlumaczenie.png}
\caption{Diagram przedstawiający proces tłumaczenia}
\end{figure}
\begin{enumerate}
\newpage
...
\item Realizacja tłumaczenia - przydzielony do tłumaczenia ekspert przystępuje do jego realizacji zaznaczając to na pasku stanu i przechodząc do etapu "in progress". Ekspert wykonując swoje zadanie może (a nawet powinien w celu zachowania bezpieczeństwa) po każdym etapie swojej pracy umieścić pliki z cząstkowym rozwiązaniem. Takie pliki powinny się znaleźć w katalogu "Pliki Docelowe" (ang. Target Files).
...
Ostatecznie projekt ten wykorzystujemy do ograniczania dostępu do konkretnych funkcji oferowanych przez stronę serwera aplikacji lub danych przez nie zwracanych. Osiągamy to przy użyciu systemu adnotacji, które umieszczone przed definicją metody definiują dla niej cechy bezpieczeństwa. W ten sposób określamy, czy użytkownik może wywołać tę funkcję na podstawie jego roli, bądź przy użyciu zdefiniowanych przez siebie warunków. W podobny sposób możemy przefiltrować kolekcję obiektów zwracanych przez funkcję. 

\section{Google Web Toolkit (ExtGWT)}

Google Web Toolkit to framework umożliwiający tworzenie skomplikowanych aplikacji opartych o przeglądarkę. GWT rozdziela samą aplikację na część serwera pisaną w języku Java, oraz część kliencką uruchamianą w przeglądarce, komunikującą się z serwerem za pomocą technologii AJAX. Ta druga część tworzona jest w języku JavaScript bądź Java, tak jak w naszej aplikacji. Sam sposób komunikacji tych dwóch części został dokładniej opisany w dziale \ref{r:spring_gxt}. Aplikacje tworzone w ten sposób przerzucają odpowiedzialność za jej wykonanie na barki przeglądarki internetowej uniezależniając swoje działanie od środowiska (systemu operacyjnego, innego oprogramowania) i umożliwiając uruchamianie bez dodatkowej instalacji.
...
Projektowanie wyglądu aplikacji możliwe było także z użyciem UIBindera, w którym komponowanie strony odbywa się z użyciem plików .xml. Pomimo tego, że prawdopodobnie umożliwiłoby to obejście problemów z zarządzaniem interfejsem, na które czasami się natykaliśmy, to jednak nakład pracy włożony w naukę obsługi tego narzędzia przekroczyłby wartość oczekiwanych z tego zysków.

\section{Hibernate}
Hibernate jest najpopularniejszą biblioteką mapowania obiektowo-relacyjnego przeznaczoną dla języka programowania Java.
Zapewnia ona przede wszystkim translację danych pomiędzy relacyjną bazą danych, a światem obiektowym. 
Opiera się na wykorzystaniu opisu struktury danych za pomocą języka XML lub adnotacji, dzięki czemu można mapować obiekty, 
...
w prosty sposób całej hierarchii klas na język bazy danych. Do każdej klasy z hierarchii należy stworzyć odpowiadający jej plik XML, 
...
\begin{itemize}
\item czy kolekcje będą leniwie wyjmowane (opcja lazy)
\item relacje między poszczególnymi klasami (wiele do wielu, jeden do wielu, itp.)
\item czy przy zapisywaniu/usuwaniu obiektu również obiekty, które są z nim powiązane będą zapisywane/usuwane (opcja cascade)
...
Poniżej przedstawiono przykładowe mapowanie klasy projekt w naszym systemie, w którym możemy zauważyć, że każdy 
projekt ma swój zbiór menadżerów, ekspertów oraz freelancerów. Funkcja lazy = true przy tych zbiorach mówi, że 
kolekcje te będą wyjmowane leniwie, co oznacza, że prosząc o projekt dostaniemy ten projekt bez zbiorów pracowników w nim pracujących.
Aby wyciągnąć projekt z jego zbiorami należy się do nich odwołać, kiedy projekt został już wyjęty z bazy danych i ciągle mamy otwartą sesję w 
Hibernate, wówczas zostaną również pobrane dodatkowe dane. Ponadto w poniższym fragmencie kodu widzimy relacje pomiędzy poszczególnymi klasami, 
na przykład pracownik może nadzorować wiele projektów ale jeden projekt może mieć jednego nadzorcę (relacja wiele do jednego).
\bigskip
%\begin{verbatim}
\lstset{numbers=left, language=XML, stepnumber=1, frame=single, breaklines=true,basicstyle=\footnotesize,}
\begin{lstlisting}
<?xml version="1.0"?>
...
Jest to obiekt, który zapewnia interfejs do bazy danych jednocześnie nie ujawniając jej szczegółów. Głównym celem stosowania DAO jest
...
Pierwszym argumentem za użyciem tego właśnie systemu jest fakt, że każdy z nas miał już doświadczenie w pracy z tym narzędziem, niektórzy całkiem świeże. Pozostali spotkali się z nim chociażby w czasie jednego z uczelnianych przedmiotów.
...
\section{Gerrit}
Gerrit to wykorzystywane przez nas narzędzie do rewizji kodu. Powstało jako narzędzie rozwijane przez Google na potrzeby Androida. Jako sprawdzone już narzędzie Open Source okazał się dobrym wyborem.\\
...
\section{Jenkins}\label{r:jenkins}
...
Jira może zostać zintegrowana z innymi produktami Atlassiana, a w szczególności wykorzystywanym przez nas Confluencem.
...
realizacji projektu.
Postanowiliśmy, jednakże wykorzystać produkt Atlassianowy, ponieważ część członków zespołu pracowała pod tymi narzędziami, Jira mogła być łatwiej zintegrowana
...
W naszym projekcie był on wykorzystywany w szczególności podczas etapu projektowania systemu oraz przy tworzeniu dokumentacji.
...
\item \textbf{FindBugs Eclipse Plugin} - wtyczka pozwalające na używanie narzędzia FindBugs. Pozwala ono na wykrycie drobnych i nie tylko błędów, łatwych do przeoczenia pomyłek lub naruszających dobre nawyki pisania w języku Java elementów kodu. Pozwala to na zmniejszenie ilości problemów, których w łatwy sposób można uniknąć.
\item \textbf{Eclipse JBoss Plugin} - wtyczka pozwalająca na zainstalowanie narzędzi Hibernate Tools do pracy z Hibernatem, jak również JBoss Maven Hibernate Configurator do zintegrowania współpracy Mavena z Hibernatem
\end{itemize}
...
\begin{figure}[ht!]
\includegraphics[width=0.95\textwidth]{resources/data.png}
\caption{Model danych zawierający wszystkie etapy przebiegu realizacji projektu}
\end{figure}

Reprezentacja użytkowników oraz stworzony przez nas system ról zostały szczegółowo opisane w punkcie \ref{r:uzytkownicy}. 
...
Szczegółowy opis procesu został opisany w punkcie \ref{r:aplikacja}. Podczas projektowania schematu rozważaliśmy różne koncepcje.
\begin{itemize}
...
Chcieliśmy w prosty sposób przedstawić informację o tym, co ma zrealizować dane zlecenie oraz pozostałe dane na temat tego zlecenia (deadline, zapłata, rodzaj tłumaczenia, dostarczone pliki).
...
Postanowiliśmy zatem, aby potraktować zarówno Ofertę, jak i Projekt jako narzędzia, służące do zarządzania zleceniem, których poszczególne etapy (części) znajdują się w liście Prac.
...
Ponadto Praca jest współdzielona przez Ofertę jak i Projekt (nie ma oddzielnych instancji).
...
który zajmował się przydzielaniem Zadań do pracowników pracujących przy danym projekcie. Do projektu pracownicy byli przydzielani przez kierownika zlecenia.
...
Miało to być realizowane nieformalnie.
\item Jako przykład bardziej szczegółowego problemu jaki napotkaliśmy było stworzenie systemu typów tłumaczeń (chodziło o rozróżnienie tłumaczeń ustnych, sądowych, technicznych, itp.).
Pierwszy pomysł przewidywał stworzenie odpowiednich enumeratorów. Uniemożliwiałoby to jednak rozszerzenie - dodanie nowego rodzaju wymagałoby zmian w kodzie. 
Postanowiliśmy zatem stworzyć oddzielną klasę TranslationType posiadającą odpowiednie informacje. Kierownik (administrator) za pomocą aplikacji mógłby dzięki temu w łatwy sposób dodać nowy typ.
\end{itemize}
...
\section{System klas i uprawnień użytkowników}\label{r:uzytkownicy}
...
Protokół ten jest przeznaczony do korzystania z usług katalogowych za pośrednictwem protokołu internetowego. LDAP obsługuje podstawowe operacje pozwalające na logowanie, wyszukanie wpisów katalogowych, dodawanie, usuwanie, sprawdzanie i modyfikowanie wpisów oraz kilka innych. Operacje te umożliwiają przechowywanie w systemie zarządzającym uwierzytelnianiem (ang. authentication) praktycznie dowolnych danych i pozwalają na unifikację systemu logowania dla wielu współdziałających aplikacji.
...
Wszystkie te klasy tworzą hierarchię klas użytkowników rozszerzając abstrakcyjną klasę użytkownika zawierającą informacje o loginie i haśle użytkownika, liście jego ról w systemie oraz ewentualnie jego dodatkowy opis.

\paragraph{System ról}
System klas użytkowników nie wyczerpuje wszystkich typów użytkowników, które miałyby być rozróżniane i obsługiwane przez naszą aplikację. Ponadto nie nadaje się on do wyznaczania uprawnień użytkownika. Dlatego każdemu użytkownikowi przypisaliśmy listę ról, które pełni on w systemie. Jest to całkiem oczywisty wybór, gdyż ten sposób opisywania uprawnień użytkownika jest powszechnie stosowany i w szczególności wpasowuje się w wymagany przez Spring interfejs. Pozwala on także na filtrowanie określonych typów użytkowników w systemie np. w momencie, gdy potrzebujemy wyświetlić użytkownikowi tylko określony ich rodzaj.

\section{Problemy z użytymi narzędziami}
\subsection{Spring + ExtGWT}\label{r:spring_gxt}
Pierwszym krokiem od którego musieliśmy zacząć implementację aplikacji było zintegrowanie wykorzystywanych przez nas technologii. Z trzech podstawowych (ExtGWT, Spring, Hibernate) nie mieliśmy jeszcze doświadczeń z Google Web Toolkit i to w pierwszej kolejności nim należało się zająć.
...
%\begin{verbatim}
\lstset{numbers=left, language=Java, stepnumber=1, frame=single, breaklines=true,basicstyle=\footnotesize,}
\begin{lstlisting}
...
wzięliśmy się za integracje ze sobą wszystkich grup. W między czasie pisaliśmy odpowiednie testy, w których staraliśmy się pokryć wszystkie możliwe 
przypadki, jednocześnie wykrywając i naprawiając pojawiające się błędy. Zbiór testów powstał dla każdej klasy mapowanej do bazy danych.\\\\
...
obiekty wysyłane za pomocą kolejek RPC muszą być serializowalne, to znaczy, że klasa opisująca dany obiekt musi implementować interfejs 
...
i wyświetlić informacje, które ze sobą ten obiekt niesie. Niestety test się nie powiódł. Patrząc na konsole trybu developement GWT dowiedzieliśmy 
...
zaskoczył, ponieważ klasa którą chcieliśmy przesłać była serializowalna i nie zawierała żadnego typu \textit{'org.hibernate.collection.PersistentSet'}.
Wobec tego zaczęliśmy szukać błędu, który zwróciła konsola GWT, w sieci. Okazało się, że obiekty wyciągane przez Hibernate'a z bazy danych są 
...
znaczenie, że do czasu kiedy obiekt jest gotowy do przesłania nie jest tym samym obiektem, który kompilator myślał, 
...
Bogatsi o nową wiedzę zaczęliśmy szukać rozwiązania trapiącego nas problemu, niestety przez dłuższy czas nie byliśmy w stanie znaleźć żadnego 
rozwiązania i do zespołu wkradło się pewne zawahanie czy na pewno dobrze zrobiliśmy próbując łączyć ze sobą GWT i Hibernate. 

Na szczęście nie poddaliśmy się i jednemu z nas udało się znaleźć rozwiązanie naszego kłopotu. Remedium okazał się wzorzec projektowy Data Transfer Object.
...
obiektów między serwerem i klientem oraz zmusiło nas to do myślenia nad strategiami kopiowania obiektów jako, że wyciągając z bazy danych pełny graf 
...
kolejnych komponentów systemu. Ostatecznie jednak, największy nakład pracy nad projektem skupił się w dwóch, około dziesięciodniowych odcinkach czasowych:
ferii zimowych oraz długiego weekendu majowego, kiedy to pozostałe obowiązki naukowe drużyny zostały tymczasowo odłożone na bok.\\

Pomimo pewnego rodzaju perturbacji, które pojawiały się w trakcie implementacji systemu, drużyna jednogłośnie stwierdziła,
że pomimo skokowego nakładu pracy, chwilami sięgającego bardzo wysokiego poziomu, byliśmy dobrze zorganizowaną grupą.
Dzięki temu wersja RC naszego projektu pojawiła się już 23 maja 2012. 
...
\subsection{Przygotowanie dokumentacji}
...
się poszczególni członkowie. Poniżej przedstawiono nad czym pracował każdy z członków i jaki był jego wpływ na nasz system. 

\paragraph{Krzysztof Gogolewski}
Krzysztof w początkowej fazie pracy zajmował się implementacją klas występujących w systemie, które miały być przechowywane w bazie 
danych. Następnie przystąpił do etapu mapowania obiektowo-relacyjnego klas, które zostały stworzone. Podczas tego etapu starał się, aby 
przyszłe zapytania do bazy danych wyciągały jedynie niezbędne w danej sytuacji informacje, na przykład obiekt nie zawraca wszystkich swoich 
kolekcji, kiedy nie jest to potrzebne (mechanizm leniwego wyjmowania w Hibernate). Kolejnym zadaniem było zaimplementowanie obiektów DAO, 
do komunikacją z baza danych. Warto wspomnieć, że nad wszystkimi wspomnianymi etapami Krzysztof pracował wspólnie z Michałem. Później Krzysztof 
zajął się dopisywaniem funkcji bazodanowych, które były potrzebne osobom piszącym już wyższą warstwę projektu, tzn. realizowali funkcjonalności. 
Kolejną zasługą Krzysztofa było napisanie skryptu, który pozwalał zapisywać stan bazy danych do pliku oraz z tego pliku wczytywać dane do bazy 
danych. Na koniec, Krzysztof pracował nad poprawą interfejsu aplikacji, starał się aby był on bardziej przyjazny oraz intuicyjny dla użytkownika. 
Wartym odnotowania jest również jego wkład włożony w pracę nad plakatem oraz logiem.

\paragraph{Karol Nienałtowski}
Najważniejszym zadaniem Karola było przygotowanie funkcjonalności dotyczących procesu tłumaczenia. Przygotował on silnie generyczną hierarchię klas składającą
się na realizacje procesu tłumaczenia, od stworzenia zlecenia, aż do zamknięcia projektu tłumaczeniowego. Dzięki starannie przygotowanej hierarchii
...
Zasługą Karola jest również wdrożenie mechanizmu internajconalizacji w naszej aplikacji oraz jej przetłumaczenie.

\paragraph{Jakub Oćwieja}
Jednym z zadań Jakuba było zaprojektowania naszego systemu w takim sposób, aby był on bezpieczny. Zapewnił on bezpieczne logowanie, 
w oparciu o możliwości jakie daje Spring oraz zapewnił obustronne filtrowanie danych, zarówno po stronie klienta jak i serwera, 
aby nie dopuścić do przekazania bądź wyświetlenia danych użytkownikowi, który nie jest do tego uprawniony. Skoro natomiast jesteśmy 
już przy użytkownikach, należało zaimplementować hierarchię użytkowników, omówioną we wcześniejszych rozdziałach tej pracy. 
Powstanie tej hierarchii to także zasługa Jakuba, jej sprawna realizacja pozwoliła na efektywne zarządzanie użytkownikami systemu i 
zapewniła wysokie bezpieczeństwo aplikacji. Jakubowi zawdzięczamy również stworzenie interfejsu do tworzenia oraz edycji użytkowników w 
systemie. Jego zasługą jest również mechanizm zarządzania plikami po stronie serwera oraz mechanizm stronicowania, do porcjowania przesyłanych 
danych między klientem i serwerem. Na koniec Jakub zajmował się również tworzeniem loga i plakatu.

\paragraph{Michał Sabaciński}
Michał, podobnie jak Krzysztof, zajmował się na początku implementowaniem klas występujących w systemie oraz ich odwzorowaniem na bazę danych 
za pomocą mapowania obiektowo-relacyjnego. Kolejnym etapem jego pracy była implementacja obiektów DAO, które miały zapewnić niezbędny interfejs do 
bazy danych. Kolejnym zadaniem Michała było napisanie testów do wyżej wymienionych obiektów DAO. Następnie zaimplementował funkcjonalności dotyczące 
administracji systemu, do których należało między innymi zarządzanie językami translacji, walutami dostępnymi przy realizacji płatności, tworzenie 
obiektów translacji dla tłumaczy. Zasługą Michała jest również stworzenie strony użytkownika, na której znajdują się szczegółowe informacje o danym 
użytkowniku. Na koniec Michał zajmował się mechanizmem zarządzania plikami po stronie klienta oraz tłumaczeniem części aplikacji dla mechanizmu 
internacjonalizacji.
...
Podobna sytuacja miała miejsce wśród członków zespołu Pengurus. Szczególnie w trakcie etapu implementacji lider zespołu starał się dobierać zadania do 
...
Każdy z członków zespołu spotykał się z pełnym przebiegiem tworzenia aplikacji, począwszy od pomysłu, udokumentowania projektu, aż po ostatnie linie kodu 
...
systemu na tyle generycznego, że potrafiłby się dostosować do obsługi szerokiej gamy biur. 
...
\begin{itemize}
	\item \textit{/pengurus-prezi} - folder zawierający prezentację licencjacką stworzoną za pomocą aplikacji Prezi
...
\begin{itemize}
\item PN-EN 15038 : 2006 Usługi tłumaczeniowe - Wymagania dotyczące świadczenia usług
