...
\kierownictwo{dr Janusza Jabłonowskiego\\Instytut Informatyki}
\opiekun{mgr Michała Możdżonka}
...
Klient posiada login, dane kontaktowe oraz dane służace do przygotowania umowy i faktur. W przypadku Klienta biznesowego każdy, z jego pełnomocników posiada dane osobowe i kontaktowe. 
...
Aby wyciągnać projekt z jego zbiorami należy się do nich odwołać, kiedy projekt został już wyjęty z bazy danych i ciągle mamy otwartą sesję w 
...
Reprezentacja użytkowników oraz stworzony przez nas system ról zostały szczegółowo opisnae w punkcie \ref{r:uzytkownicy}. 
...
Chcieliśmy w prosty sposób przedstawić informację o tym, co ma zrealizować dane zlecenie oraz pozostałe dane na temat tego zlecenia (deadline, zapłata, rodzaj tłumaczenia, dotarczone pliki).
...
Postanowiliśmy zatem stworzyć oddzielną klasę TranslationType posiadającą odpowiednie informacje. Kierownik (admnistrator) za pomocą aplikacji mógłby dzięki temu w łatwy sposób dodać nowy typ.
\end{itemize}
...
danych. Na koniec, Krzystof pracował nad poprawą interfejsu aplikacji, starał się aby był on bardziej przyjazny oraz intuicyjny dla użytkownika. 
